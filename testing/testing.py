...
class DataExtractor:
    def extract_links(self, file_path):
        if "no links" in file_path:
            return []
        return [("http://example.com", "Example Link")]
...
class PPTLoader:
    def extract_images(self, file_path):
        return [{"resolution": "1920x1080", "format": "JPEG"}]
...
class TestFileProcessing(unittest.TestCase):
    ...
    def test_pdf_loader_valid(self):
        ...
        result = pdf_loader.load("sample.pdf")
        self.assertEqual(result, "PDF file loaded successfully")

    def test_pdf_loader_invalid(self):
        pdf_loader = PDFLoader()
        with self.assertRaises(ValueError) as context:
            pdf_loader.load("corrupted.pdf")
        self.assertEqual(str(context.exception), "Invalid PDF file")

    def test_docx_loader_extract_text(self):
        docx_loader = DOCXLoader()
        extracted_text = docx_loader.extract_text("sample.docx")
        self.assertEqual(extracted_text, "Extracted text from DOCX")

    def test_data_extractor_hyperlink_extraction(self):
        data_extractor = DataExtractor()
        links = data_extractor.extract_links("doc_with_links.docx")
        self.assertEqual(len(links), 1)
        self.assertEqual(links[0], ("http://example.com", "Example Link"))

    def test_data_extractor_no_links(self):
        data_extractor = DataExtractor()
        links = data_extractor.extract_links("doc_with_no_links.docx")
        self.assertEqual(links, [])

    def test_ppt_loader_extract_images(self):
        ppt_loader = PPTLoader()
        images = ppt_loader.extract_images("sample.ppt")
        self.assertEqual(len(images), 1)
        self.assertEqual(images[0]['resolution'], "1920x1080")
        self.assertEqual(images[0]['format'], "JPEG")

    def test_file_storage_save_text(self):
        file_storage = FileStorage()
        text_to_save = "Extracted text"
        file_storage.save_to_file("output.txt", text_to_save)

        with open("output.txt", 'r') as f:
            content = f.read()
            self.assertEqual(content, text_to_save)

    def test_sql_storage_store_hyperlinks(self):
        sql_storage = SQLStorage()
        links = [("http://example.com", "Example Link")]
        stored_count = sql_storage.store_links(links)
        self.assertEqual(stored_count, 1)

    def test_data_extractor_table_extraction(self):
        data_extractor = DataExtractor()
        tables = data_extractor.extract_tables("doc_with_tables.docx")
        self.assertEqual(len(tables), 2)
        self.assertEqual(tables[0], ["Header1", "Header2"])

    def test_sql_storage_store_tables(self):
        sql_storage = SQLStorage()
        tables = [["Header1", "Header2"], ["Row1Col1", "Row1Col2"]]
        stored_count = sql_storage.store_tables(tables)
